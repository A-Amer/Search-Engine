...
//           Logger.getLogger(DBmanager.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public boolean AddText(String url,String text,String title){
        CallableStatement cstmt;
        boolean x;
        try {
            cstmt = CONNECTION.prepareCall("{call AddSnippet(?,?,?)}");
...
//           Logger.getLogger(DBmanager.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void DeleteText(String url){
        Statement statement = null;
        try {
            String Query = "Delete FROM guest.Snippet WHERE url='"+url+"';";
            statement = CONNECTION.createStatement();
